...
def dqn_algo(env_name='CartPole-v1', render=False, device='cpu',
             steps_per_epoch=200, epochs=200, max_ep_len=500, replay_size=int(1e6), batch_size=100,
             start_steps=10000, update_after=200, update_every=50,
             hidden_sizes=(32, 32, 32), activation=torch.nn.ReLU, target_freq=5,
             gamma=0.99, epsilon_start=0.9, epsilon_final=0.1,
             num_test_episodes=10, max_test_ep_len=1000, save_freq=5, save_path='model/'):
    ...
    ag = dqnAgent(obs_dim, act_dim, act_mesh, hidden_sizes, activation, device=device,
                  gamma=gamma, test_num_episodes=num_test_episodes, test_max_ep_len=max_test_ep_len)
    ...
    for t in range(total_steps):
        ...
        if (t + 1) % steps_per_epoch == 0:
            ...
            epsilon = epsilon_start - epoch / epochs * (epsilon_start - epsilon_final)

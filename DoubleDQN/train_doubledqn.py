...
import gym
import torch
from DoubleDQN.DoubleDQN import doubleDQNAgent
...
def doubledqn_algo(env_name='CartPole-v1', device='cpu',
                   ...
                   gamma=0.99, epsilon_start=0.9, epsilon_final=0.1, polyak=0.995, save_freq=5, save_path='model/'):
    env = gym.make(env_name)

    obs_dim = 6
    act_dim = 2

    ag = doubleDQNAgent(obs_dim, act_dim, device, gamma, polyak)
    ...
    PATH = save_path + 'doubledqn_cuda.pt' if device is not 'cpu' else save_path + 'doubledqn_cpu.pt'
